...
const nextConfig: NextConfig = {
  ...
  images: {
    // Uploaded media lives on a different origin to the app: SeaweedFS in development, R2
    ...
    //
    // Two reasons. In development the browser reaches the bucket on localhost, which the
    // Next server inside its container cannot resolve, so a server-side fetch would fail.
    // And in production R2 already sits behind Cloudflare's CDN, so re-encoding through a
    // Node process would duplicate work and add egress for nothing
    remotePatterns: [
